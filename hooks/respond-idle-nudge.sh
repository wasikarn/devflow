...
if [ -d "$TASKS_DIR" ]; then
  ...
  if [ -n "$PENDING" ]; then
    echo '{"decision": "block", "reason": "You have pending threads to fix. Continue fixing your assigned threads and message the team lead when done."}' >&2
    exit 2
  fi
fi

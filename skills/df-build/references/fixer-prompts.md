...
PLAN CONTEXT: Read `{artifacts_dir}/devflow-context.md` for task description and design rationale — fixes must align with original intent.
...
5. **`{validate_command}`**: same command as used by workers — from `devflow-context.md` `validate:` field.

---
name: onboard
description: "Bootstrap the current project into the anvil ecosystem — scaffold hard-rules.md and build artifact directories. Use once per new project before the first build or review invocation. Run this first when starting a new project, setting up anvil for the first time, or when hard-rules.md is missing. Triggers: onboard, onboard project, bootstrap anvil, setup anvil, first time setup, initialize anvil, scaffold hard rules, new project setup."
...
Bootstraps the current project into the anvil ecosystem:
...
- Run from within the target project's root directory (not the anvil plugin directory)
- Plugin must already be installed: `claude plugin install anvil`
...
- **Does not install the plugin** — onboarding sets up per-project configuration (`.claude/skills/review-rules/`), not the plugin itself. Run `claude plugin install anvil` separately before invoking any anvil skill.
...
- **Runs as an isolated subagent (`context: fork`)** — delegated entirely to the `project-onboarder` agent. The agent detects the project root from the working directory. Invoke from within the target project's root, not from the anvil plugin directory.

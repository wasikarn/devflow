...
`feature-dev:code-reviewer` applies TypeScript advanced type principles (branded types, discriminated unions, type guards — NO `any`), Clean Architecture DDD principles (domain isolation, hexagonal ports/adapters, value objects), and Effect-TS patterns (`Effect.gen`, `Layer`, `pipe`). All agents use confidence scoring: 90–100 → 🔴, 80–89 → 🟡. Hard Rules bypass — always 🔴.
